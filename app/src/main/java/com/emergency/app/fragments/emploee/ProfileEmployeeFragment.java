...
import com.emergency.app.databinding.FragmentProfileEmployeeBinding;
...
public class ProfileEmployeeFragment extends Fragment implements View.OnClickListener , MediaBottomSheetDialog.MediaBottomSheetListener{

    FragmentProfileEmployeeBinding bind;
    private final int PICK_IMAGE_REQUEST = 1;
    private final int CAPTURE_IMAGE_REQUEST = 2;
    private Uri filePath;
    private AndroidPermission objAndroidPermission;
    private boolean isChooseImage,isChooseImageFromGalllery;
    ...
    public static ProfileEmployeeFragment newInstance() {
        ProfileEmployeeFragment fragment = new ProfileEmployeeFragment();
        return fragment;
    }
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        bind = DataBindingUtil.inflate(inflater, R.layout.fragment_profile_employee, container, false);
    ...
    private void initViews()
    {
        try {
            ...
            adapterJob=new CustomArrayAdapter(R.layout.item_spinner_layout,getActivity(),jobsList);
            ...
            bind.lyReviews.setOnClickListener(this);
    ...
    public void onClick(View view) {
        switch (view.getId())
        {
            ...
            case R.id.lyReviews:
                try {
                    ReviewsDialog reviewsDialog = ReviewsDialog.newInstance(user.getId());
    ...
    private void setUserData()
    {
        try
        {
            ...
            bind.edtPrice.setText(user.getPrice());
            ...
            {
                if(user.getJob().equals(jobsList.get(i)))
                {
                    bind.spJob.setSelection(i);
                }
            }
            //reviews and rate
            bind.tvOrders.setText(user.getOrders()+"");
            bind.tvRate.setText(user.getRate());
            bind.tvReviews.setText(user.getReviews()+"");
        }
        catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void update()
    {
        try
        {
            ...
            DatabaseReference mdata=FirebaseDatabase.getInstance().getReference().child(AppConfigHelper.EMPLOYEE_CHILD).child(AppConfigHelper.PROFILE_CHILD).child(String.valueOf(user.getId()));
            ...
            mdata.child(AppConfigHelper.PRICE_FIELD).setValue(bind.edtPrice.getText().toString());
            mdata.child(AppConfigHelper.JOB_FIELD).setValue(bind.spJob.getSelectedItem().toString());
            ...
            if(filePath!=null)
            {
                StorageReference ref = storageReference.child(AppConfigHelper.EMPLOYEE_CHILD+"/"+user.getId());
            ...
            user.setPrice(bind.edtPrice.getText().toString());
            user.setJob(bind.spJob.getSelectedItem().toString());
